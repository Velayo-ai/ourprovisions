...
- **OurBanner columns — migration 024 (APPLIED to DEV 2026-07-19; prod pending):** `photo_path text`, `photo_position_x smallint default 50`, `photo_position_y smallint default 50`, `photo_zoom smallint default 100`, `banner_wordmark text default 'large'` — with CHECK constraints (positions 0–100, zoom 100–320, `banner_wordmark in ('large','small','hidden')`). **`photo_path`, not `photo_url`** — the storage object path (URLs embed project refs + expiry and break across dev/prod; resolve at read time). **No RLS change needed** — the existing `households` policy is `is_member_of(household_id)` for all commands, so any member can already update these (the "group sets the look" model falls out of what's there). All five are **household-scoped state** and must swap the instant `activeHouseholdId` changes (per the filter-reset rule). See the OurBanner note below + `docs/specs/active/SPEC_household_photo_header.md`. **Refined 2026-07-18 by `docs/specs/active/SPEC_household_management_phase1.md` (Phase I):** these columns are edited in the **Edit-household sheet** the redesign's pencil opens; **Delete household moves into that sheet** (creator-only danger zone), so the RLS stays `is_member_of` for the photo/name/wordmark edits while Delete remains guarded creator-only inside `delete_household`. The migration file carried a one-char verify-block bug (`and X like … or Y like …` — `AND` binds tighter than `OR`); corrected to `and (…)` 2026-07-18. **APPLIED to dev 2026-07-19** (verify: 5 columns, 4 CHECK constraints, 4 storage policies, 13 households on the espresso default); prod pending. **Client reads these columns best-effort** — a deploy that lands before the migration degrades to the espresso header instead of hard-erroring on the missing column; `photo_path` resolves to a **signed URL** (private bucket), re-resolved on `activeHouseholdId` switch and on save (1h TTL — a watch item).
...
### Splash / entry scene — auto-play, readiness-gated, wordmark-relative, runtime-measured + hidden-target hand-off *(2026-07-23 — BUILT + on dev; `SplashScreen` in `App.js`; per `docs/specs/built/SPEC_splash_vessel_identity_v2.md`)*
The animated splash is the **first mounted React view**, NOT the native launch screen — Apple forbids animating the true launch screen, so on native iOS (1.0) it stays static and the animated scene lives here. It **auto-plays on cold start** — no tap gate (the "Tap to enter" gesture existed only to satisfy audio autoplay, and audio moved to Trip Complete). The entry plays three meaning-grouped motions (horizon → vessel → house), then a simple surfacing dissolve. Load-bearing facts:
...
The 2026-07-18 design session built the real feature and **overrides two things above.** Spec: `docs/specs/active/SPEC_household_photo_header.md`; mockup of record (tiebreaker): `docs/mockups/mockup_ourbanner.html`. Migration 024, authored not applied.
