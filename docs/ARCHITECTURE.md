...
### Splash launch experience — post-launch React view, readiness-gated, runtime-measured hand-off *(2026-07-22 — specced, not yet built; per `docs/specs/active/SPEC_splash_vessel_identity.md`)*
The animated splash is the **first mounted React view**, NOT the native launch screen — Apple forbids animating the true launch screen, so on native iOS (1.0) it stays static and the animated scene lives here, exactly as it does today (`SplashScreen` in `App.js`). Three load-bearing build facts:
- **Dismissal is readiness-gated, not a timer.** The current `SplashScreen` fires `onDone` on a fixed timer; replace with: intro-minimum-elapsed **AND** `!loading` (the household/provisions load flag already in scope), with a **~1.8–2.2s minimum** (a fast load still shows the full intro rather than flashing) and a **~5s failsafe maximum** (a stuck load never traps the user). The dissolve fires on ready, not on a clock.
- **The wordmark hand-off measures its target at runtime — never hardcode.** On dissolve the splash wordmark travels to the **actual header title element's** `getBoundingClientRect()` + computed font-size (give that element a ref), so the title *becomes* the header rather than being replaced. This is what makes the seam invisible across viewport widths and independent of whether "Our" shows. Verify at 390px & 430px. Fallback: a robust "lift & fade" (Option 1) — a clean lift beats a visibly-misaligned hand-off.
- **Paired header standardization:** for the hand-off to land on the same words, the header must render "*Our* Provisions" (today the no-photo header renders bare "Provisions"). Confirm with Dan, then standardize. Asset note: the footer V-mark needs a **true transparent** SVG/PNG — the existing `VELAYO_LOGO_WHITE`/`_TEAL` PNGs have solid backgrounds and will box; the particle wash renders as **canvas/WebGL** (BVI-toned points, additive blend, rising bottom-up), not CSS. The brand-deck PDF (`velayobranddeck__March_28_2026.pdf`) is corrupted/unreadable — needs a clean re-export.
