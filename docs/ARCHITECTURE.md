...
- **KNOWN GAP (tracked in ROADMAP NOW):** Browse `selectedCategories`/`stapleFilter` do **not** yet reset on `activeHouseholdId` change — a household-scoped-state leak surface (same class as the join-banner / invite-link leaks). The intended "commit 0" filter-reset was missed in the build; the severe invisible-stale-filter form is defused by phase-reset-to-0 (pills always re-shown after a switch), but filters carry over visibly. Fix pending. Reference mockup: `docs/mockups/cycle_dual_readout.html`; build trail: `docs/DECLUTTER_BUILD_HANDOFF.md`.
