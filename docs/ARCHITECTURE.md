...
> **No Supabase migration tracker exists on this project** — `supabase_migrations.schema_migrations` does not exist on prod. Migrations are applied by hand in the SQL editor, never via CLI. Filing numbers are a folder convention, not load-bearing. Record applied state by querying the live catalog (`pg_proc`, `pg_policies`), not by trusting the folder or any doc. Write all migrations idempotent (`drop ... if exists` / drop-and-recreate). *(On disk the high-water mark is now `020` (018/019/020 committed as records of hand-applied migrations); the `009`–`012` gap and the `007` collision — `docs/007_dev_restore_role_grants.sql` vs `migrations/007_finish_authorize_sweep.sql` — still stand; see ROADMAP reconciliation item.)*
...
| — | **referral attribution** *(deferred, beta 2026-07-14)* | A referrer code captured at the link layer (`?ref=<code>`) at signup. Per "onboarding data belongs to the user and is never conflated with the immutable `beta_signups` intake record," this lands in its **own column/table**, not folded into intake telemetry. Referral = advocacy (no token/grant), structurally distinct from a household-invite (token → membership → write access). Rewards logic Phase 4. |
