...
| # | Feature | Notes |
|---|---|---|
| — | **Layer 2: `household_members` realtime subscription** | A removed member's own window must react live. `refreshMembers` (Layer 1, actor-side) is built. Layer 2 = Supabase realtime subscription on `household_members` filtered to the active household. On DELETE event: if the removed `user_id` is the current user → show notice + auto-switch. |
