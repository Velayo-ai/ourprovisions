...
| Jun 29, 2026 | **Two-verb workflow: `SPEC` (claude.ai produces a spec into the airlock) / `BUILD` (Claude Code implements it).** BUILD is implement-and-stop: one scoped commit, grep-before-edit, test on deployed dev, stop at dev, leave the spec in `handoff/` for SESSION END. Does NOT route/log/clear. |
