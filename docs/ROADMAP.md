...
| # | Feature | Notes |
|---|---|---|
| 1 | **Per-household staple model** *(headline — design-first)* | Fix the global-staple revert bug (found 2026-06-29): `is_staple` is a single boolean on the shared global `catalog_items` row (`is_global=true`, `household_id=NULL`), so tapping Staple on a global item paints green optimistically but has no per-household storage — the 20s catalog poll re-reads `false` and reverts to grey. Direction: a `household_staples` join table (`household_id`, `catalog_item_id`) where staple-status is row-presence, RLS-gated on `is_member_of`; rewrite `toggleStaple` write + the catalog read (`refreshCatalog` ~App.js:1189) + the staple filter (~App.js:461); decide whether custom items (real `household_id`) keep the column. **Spec-before-code** — produce the mockup/spec, then BUILD. Prod-leak check (`is_global=true and is_staple=true`) returned 0 rows — no cross-household leak has occurred. |
