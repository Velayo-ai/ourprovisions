...
export default function ShoppingListApp() {
  ...
  const [prices, setPrices] = useLocalStorage("op_prices", buildDefaultPrices(DEFAULT_CATEGORIES));
  const [view, setView] = useLocalStorage("op_view", "input");
  const [checked, setChecked] = useLocalStorage("op_checked", {});
  // BUG FIX: budget goal stored in localStorage
  const [budgetGoal, setBudgetGoal] = useLocalStorage("op_budget_goal", "");
  ...
  const updateQty = (item, val) => {
    ...
    setQuantities((prev) => ({ ...prev, [item]: num }));
  };

  // BUG FIX: removeItem now correctly preserves empty default categories
  // and properly removes custom categories only when their last item is gone
  const removeItem = (itemName) => {
    setCategories(prev =>
      prev
        ...
        .filter(cat => {
          // Always keep default categories even if empty
          if (DEFAULT_CATEGORY_NAMES.has(cat.name)) return true;
          // Only keep custom categories if they still have items
          return cat.items.length > 0;
        })
    );
    setQuantities(prev => { const n = { ...prev }; delete n[itemName]; return n; });
    setPrices(prev => { const n = { ...prev }; delete n[itemName]; return n; });
    setChecked(prev => { const n = { ...prev }; delete n[itemName]; return n; });
  };
  ...
  const commitPrice = (itemName) => {
    ...
    if (!isNaN(val) && val >= 0) {
      setPrices(prev => ({ ...prev, [itemName]: parseFloat(val.toFixed(2)) }));
    }
  ...
  const saveBudget = () => {
    ...
    if (!isNaN(val) && val > 0) {
      setBudgetGoal(parseFloat(val.toFixed(2)));
  ...
  const budgetNum = budgetGoal !== "" ? parseFloat(budgetGoal) : null;
  ...
  const toggleChecked = (item) => setChecked((prev) => ({ ...prev, [item]: !prev[item] }));
  const clearAll = () => { setQuantities({}); setChecked({}); setView("input"); };
              ...
              <div className="category-block" key={cat.name}>
                ...
                <div className="items-grid">
                  {cat.items.map((item) => {
                    ...
                    return (
                      <SwipeToRemove key={item.name} onRemove={() => removeItem(item.name)}>
                        <div className={`item-row ${qty > 0 ? "has-qty" : ""}`}>
                          <div className="item-top">
                            <span className="item-name">{item.name}</span>
                            <div className="qty-controls">
                              <button className="qty-btn" onClick={() => updateQty(item.name, qty - 1)}>−</button>
